    ...
    <div className="flex flex-col gap-2 bg-card border border-dashed border-border/80 p-2 rounded-xl w-full overflow-hidden opacity-70">
      ...
      <div className="px-2 mt-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground line-clamp-2">
          Something awesome is currently being built. Exploring new ideas and
          technologies. Stay tuned for updates.
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 px-2 mt-4 h-5">
        {["Tech", "Stack", "Hidden"].map((label) => (
          <span
            key={label}
            className="text-[10px] font-mono text-muted-foreground uppercase bg-muted/50 px-2 rounded-md border border-border/50 h-full flex items-center"
          >
            {label}
          </span>
        ))}
